...
def test_severe_bleeding_returns_protocol() -> None:
    ...
    ) or "wound" in result.lower() or "blood" in result.lower()
...
def test_fracture_returns_protocol() -> None:
    ...
    assert "ice" in result.lower() or "fracture" in result.lower(
    ) or "immobilize" in result.lower()
...
def test_default_returns_protocol() -> None:
    ...
    assert "911" in result or "monitor" in result.lower(
    ) or "consciousness" in result.lower() or "calm" in result.lower()
